...
const flashMiddleware = (req, res, next) => {
    // Track if flash message were set (need to save session before redirect)
    let sessionNeedsSave = false;

    // Override req.redirect to save session before redirecting
     ...
     * - Called with 1 arg (type): retrieves and clears message of that type
     * - Called with 0 args: retrieves and clears all messages
     */
    req.flash = function(type, message) {
        // Guard: If session doesn't exist (e.g., after session.destroy()), return early to prevent errrors. Flash messages require a session to store.
        if (!req.session) {
        ...
        if (type && message) {
            // Ensure the message type's array exists
...
/**
 * Make flash function available to all templates vis res.locals
 */
const flashLocals = (req, res, next) => {
    ...
    // Messages are only consumed when a template call flash()
    res.locals.flash = req.flash;
    next();
}

/**
 * Combined flash middleware that runs both function in the correct order
 */
